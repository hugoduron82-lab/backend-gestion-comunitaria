...
export class HistorialDirectivas implements OnInit {
  ...
  cargar() {
    ...
    this.historialService.listar({
      busqueda: this.busqueda(), zona: this.filtroZona(),
      pagina: this.paginaActual(), porPagina: 15
    }).subscribe({
      next: (res) => {
      ...
      error: (err) => { console.error('Error cargando historial:', err); this.cargando.set(false); }
    });
  }

  onBusquedaChange(valor: string) {
    this.busqueda.set(valor); this.paginaActual.set(1);
    clearTimeout(this.timeoutBusqueda);
    this.timeoutBusqueda = setTimeout(() => this.cargar(), 400);
  }

  onZonaChange(valor: string) {
    this.filtroZona.set(valor ? Number(valor) : null); this.paginaActual.set(1); this.cargar();
  }

  irPagina(pagina: number) {
    if (pagina < 1 || pagina > this.totalPaginas()) return;
    this.paginaActual.set(pagina); this.cargar();
  }
