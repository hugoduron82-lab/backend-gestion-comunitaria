...
const CARGOS = [
  { id: 1, nombre: 'Presidente' },  { id: 2, nombre: 'Vicepresidente' },
  { id: 3, nombre: 'Secretario' },  { id: 4, nombre: 'Tesorero' },
  { id: 5, nombre: 'Fiscal' },      { id: 6, nombre: 'Vocal I' },
  { id: 7, nombre: 'Vocal II' },    { id: 8, nombre: 'Vocal III' },
...
export class OrganizacionesEditar implements OnInit {
  ...
  nuevoMiembro = {
    ...
    nombre_completo: '', dni: '', telefono_personal: '', fecha_inicio: ''
  };
  errorNuevo = signal('');
  ...
  ngOnInit() {
    ...
    this.catalogosService.tipos().subscribe({ next: (res) => this.tipos.set(Array.isArray(res) ? res : []) });
    this.catalogosService.zonas().subscribe({ next: (res) => this.zonas.set(Array.isArray(res) ? res : []) });
  ...
  cargarOrganizacion() {
    ...
    this.orgService.obtenerPorId(this.idOrganizacion).subscribe({
      next: (org) => {
        this.nombre = org.nombre; this.id_tipo = org.id_tipo; this.id_zona = org.id_zona;
        this.colonia_sector = org.colonia_sector || '';
        this.fecha_inscripcion = org.fecha_inscripcion?.slice(0, 10) || '';
        this.tomo = org.tomo || ''; this.folio = org.folio || '';
        this.observaciones = org.observaciones || '';
        this.cargandoOrg.set(false);
      },
      error: () => { this.error.set('No se pudo cargar la organización'); this.cargandoOrg.set(false); }
    });
  }
    ...
    this.directivaService.listar(this.idOrganizacion).subscribe({
      next: (miembros) => {
        this.directiva.set((miembros ?? []).map((m: any) => ({
          id: m.id, id_cargo: m.id_cargo,
          cargoNombre: m.cargo?.nombre || CARGOS.find(c => c.id === m.id_cargo)?.nombre || '',
          nombre_completo: m.nombre_completo, dni: m.dni,
      ...
      error: () => { this.directiva.set([]); this.cargandoDirectiva.set(false); }
    });
  }

  get cargando() { return this.cargandoOrg() || this.cargandoDirectiva(); }
  ...
  totalMiembros() { return this.directiva().length; }

  abrirFormNuevo() {
    this.errorNuevo.set('');
    const disponibles = this.cargosDisponibles();
    this.nuevoMiembro = {
      id_cargo: disponibles[0]?.id ?? null, nombre_completo: '', dni: '',
      telefono_personal: '', fecha_inicio: new Date().toISOString().slice(0, 10)
    };
    this.mostrarFormNuevo.set(true);
  }

  cancelarFormNuevo() { this.mostrarFormNuevo.set(false); this.errorNuevo.set(''); }

  guardarNuevoMiembro() {
    this.errorNuevo.set('');
    if (!this.nuevoMiembro.id_cargo || !this.nuevoMiembro.nombre_completo ||
        !this.nuevoMiembro.dni || !this.nuevoMiembro.fecha_inicio) {
      this.errorNuevo.set('Completa cargo, nombre, DNI y fecha de inicio');
      return;
    }
    this.guardandoNuevo.set(true);
    const payload = {
      id_organizacion: this.idOrganizacion, id_cargo: this.nuevoMiembro.id_cargo,
      nombre_completo: this.nuevoMiembro.nombre_completo, dni: this.nuevoMiembro.dni,
    ...
    this.directivaService.asignarMiembro(payload).subscribe({
      next: () => { this.guardandoNuevo.set(false); this.mostrarFormNuevo.set(false); this.cargarDirectiva(); },
      error: (err) => { this.guardandoNuevo.set(false); this.errorNuevo.set(err.error?.msg || 'Error al agregar el miembro'); }
    });
  }

  quitarMiembro(m: any) {
    if (!confirm(`¿Quitar a ${m.nombre_completo} (${m.cargoNombre}) de la directiva?`)) return;
  ...
  verificarAutorizacion() {
    this.authError.set('');
    if (!this.authCorreo || !this.authPassword) { this.authError.set('Ingresa correo y contraseña'); return; }
    ...
    this.auth.verificarCredenciales(this.authCorreo, this.authPassword).subscribe({
      next: (res) => {
        ...
        if (res?.usuario?.id_rol === 2) {
          this.desbloqueado.set(true); this.mostrarModalAuth.set(false);
        } else {
          this.authError.set('Estas credenciales no corresponden al Jefe de Desarrollo Comunitario');
        }
      },
      error: () => { this.verificandoAuth.set(false); this.authError.set('Correo o contraseña incorrectos'); }
    });
  }

  cancelarAutorizacion() { this.router.navigate(['/dashboard']); }

  guardar() {
    this.error.set(''); this.exito.set('');
    if (!this.nombre || !this.id_tipo || !this.id_zona || !this.fecha_inscripcion) {
      this.error.set('Completa los campos obligatorios marcados con *'); return;
    }
    ...
    const payload = {
      ...
      fecha_inscripcion: this.fecha_inscripcion, observaciones: this.observaciones
    };
    this.orgService.actualizar(this.idOrganizacion, payload).subscribe({
      next: () => this.guardarDirectivaModificada(0),
      error: (err) => { this.guardando.set(false); this.error.set(err.error?.msg || 'Error al actualizar la organización'); }
    });
  }

  private guardarDirectivaModificada(index: number) {
    const pendientes = this.directiva().filter(m => this.miembroCambio(m));
    if (index >= pendientes.length) { this.finalizar('Organización actualizada correctamente.'); return; }
    const m = pendientes[index];
    const payload: any = {
      nombre_completo: m.nombre_completo, telefono_personal: m.telefono_personal, fecha_inicio: m.fecha_inicio
    };
    if (m.dni !== m._original.dni) payload.dni = m.dni;
    this.directivaService.actualizarMiembro(m.id, payload).subscribe({
      next: () => this.guardarDirectivaModificada(index + 1),
      error: (err) => {
        this.guardando.set(false);
        this.error.set(`Los datos generales se guardaron, pero hubo un problema con "${m.nombre_completo}": ${err.error?.msg || 'Error desconocido'}.`);
      }
    });
  }
  ...
  private finalizar(mensaje: string) {
    this.guardando.set(false); this.exito.set(mensaje);
